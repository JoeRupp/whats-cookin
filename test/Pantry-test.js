...
recipeIngredientList = [
        {id: 20081, name: 'wheat flour', estCost: 213, unit: 'c', amount: '1.5'},
...
pantry = new Pantry(userIngredients, ingredientData);
  ...
  it("Should be able to get ingredient by name", () => {
    const ingredName = pantry.getIngredientName(ingredientData, userIngredients[0]);
  ...
  it("Should be able to determine cook ability", () => {
    expect(pantry.determineCookAbility(recipeIngredientList)[0].id).to.equal(20081)
    expect(pantry.determineCookAbility(recipeIngredientList).length).to.equal(2)
  });

  it("Should be able to find the missing ingredients", () => {
    expect(pantry.findMissingIngredients(recipeIngredientList)[0].id).to.equal(1123)
  });
